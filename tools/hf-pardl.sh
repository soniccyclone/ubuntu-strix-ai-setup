#!/usr/bin/env bash
# Ranged parallel fetch of a HuggingFace repo's safetensors + json + txt files, sha256-verified
# against HF's LFS oids. HF shapes per-connection throughput; 32 ranges saturate this box's link.
set -euo pipefail
REPO="${1:?usage: hf-pardl.sh <org/repo> <destdir>}"; DST="${2:?destdir}"; PARTS="${PARTS:-32}"; mkdir -p "$DST"
cd "$DST"
curl -s "https://huggingface.co/api/models/$REPO?blobs=true" | python3 -c '
import json,sys
for s in json.load(sys.stdin)["siblings"]:
    n=s["rfilename"]
    if n.endswith((".safetensors",".json",".txt",".jinja")): print(n,s["size"],s.get("lfs",{}).get("sha256",""))' > manifest.txt
